...
class SupplierViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Supplier.objects.filter(restaurant_id=restaurant_id)
...
class PurchaseInvoiceViewSet(
    ...
    def get_queryset(self):
        restaurant_id, _ = assert_restaurant_access(self.request, allowed_roles=READ_ROLES)
        return (
            PurchaseInvoice.objects.select_related("supplier", "created_by")
    ...
    def from_forecast(self, request):
        ...
        serializer = PurchaseDraftFromForecastSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        v = serializer.validated_data

        supplier = Supplier.objects.get(pk=v["supplier"], restaurant_id=restaurant_id)
        ...
        items_map = {
            ...
            for i in InventoryItem.objects.filter(
                id__in=[item_id for item_id, _ in draft_lines],
                restaurant_id=restaurant_id,
            )
        }
        ...
        for idx, (item_id, qty) in enumerate(draft_lines):
            ...
            unit_cost = (getattr(item, "cost_per_unit", Decimal("0.00")) or Decimal("0.00")).quantize(
                Decimal("0.01")
            )
