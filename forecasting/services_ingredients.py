...
def build_ingredient_plan(
    ...
    recipe_lines = RecipeLine.objects.filter(menu_item_id__in=item_ids).select_related(
        "menu_item", "ingredient"
    )
    if restaurant_id is not None:
        recipe_lines = recipe_lines.filter(
    ...
    menu_qs = MenuItem.objects.filter(id__in=item_ids).only("id", "name")
    if restaurant_id is not None:
        menu_qs = menu_qs.filter(category__restaurant_id=restaurant_id)
    menu_name_map = {m.id: m.name for m in menu_qs}
