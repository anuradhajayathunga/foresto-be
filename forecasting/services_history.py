...
def _daily_qty_map(days_back: int = 180):
...
def predict_past_days(days: int = 14, days_back: int = 180, top_n: int = 50):
    """
    Predict the last N days (including yesterday) per menu item.
    This is for comparison (actual vs predicted).
    """
    days = max(1, min(days, 90))
    model = get_model()

    start, today, qty_map = _daily_qty_map(days_back=days_back)
    end_day = today - timedelta(days=1)  # yesterday as last point
    ...
    # sort by yesterday actual (or diff) – you can change
